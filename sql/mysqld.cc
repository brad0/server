...
static void handle_socket_timeout(ulonglong now_time)
{
  if (connection_count == 0 && extra_connection_count == 0 &&
      now_time > (server_last_activity + max_idle_execution * 1000000))
  {
...
void handle_connections_sockets()
{
  ...
  while (!abort_loop)
  {
    ...
    {
      ulonglong now_time= microsecond_interval_timer();
      handle_socket_timeout(now_time);
      server_last_activity= now_time;
    }
